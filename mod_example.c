 ...
 * prefixed with 'mb_' instead of 'example_'.
 *
 * IMPORTANT NOTE
 * ==============
 *
 ...
 * data don't get lost.
 */
static apr_pool_t *mb_pool = NULL;
static apr_pool_t *mb_subpool = NULL;
 ...
 */
static mb_cfg *our_dconfig(const request_rec *r)
{
    return (mb_cfg *) ap_get_module_config(r->per_dir_config, &example_module);
}
...
{
    return (mb_cfg *) ap_get_module_config(s->module_config, &example_module);
}

/*
 * Likewise for our configuration record for the specified request.
 */
static mb_cfg *our_rconfig(const request_rec *r)
{
    return (mb_cfg *) ap_get_module_config(r->request_config, &example_module);
}
#endif
 ...
 */
static mb_cfg *our_cconfig(const conn_rec *c)
{
    return (mb_cfg *) ap_get_module_config(c->conn_config, &example_module);
}
...
static void setup_module_cells(void)
{
    ...
    if (static_calls_made == NULL) {
        static_calls_made = apr_table_make(mb_pool, 16);
    };
}
 ...
 * The list can be displayed by the mb_handler() routine.
 *
...
static void trace_add(server_rec *s, request_rec *r, mb_cfg *mconfig,
                      const char *note)
{
    const char *sofar;
    char *addon;
...
static int mb_handler(request_rec *r)
{
    ...
    dcfg = our_dconfig(r);
    trace_add(r->server, r, dcfg, "mb_handler()");
    /*
 ...
 * structure.
 */
static void *mb_create_dir_config(apr_pool_t *p, char *dirspec)
{
    mb_cfg *cfg;
    char *dname = dirspec;
    ...
    trace_add(NULL, NULL, cfg, "mb_create_dir_config()");
    return (void *) cfg;
}
 ...
 */
static void *mb_merge_dir_config(apr_pool_t *p, void *parent_conf,
                                      void *newloc_conf)
{

    mb_cfg *merged_config = (mb_cfg *) apr_pcalloc(p, sizeof(mb_cfg));
    mb_cfg *pconf = (mb_cfg *) parent_conf;
    mb_cfg *nconf = (mb_cfg *) newloc_conf;
    char *note;
     ...
     */
    note = apr_pstrcat(p, "mb_merge_dir_config(\"", pconf->loc, "\",\"",
                   nconf->loc, "\")", NULL);
...
static void *mb_create_server_config(apr_pool_t *p, server_rec *s)
{
    ...
    trace_add(s, NULL, cfg, "mb_create_server_config()");
    return (void *) cfg;
}
                                         ...
                                         void *server2_conf)
{
     ...
     */
    note = apr_pstrcat(p, "mb_merge_server_config(\"", s1conf->loc, "\",\"",
                   s2conf->loc, "\")", NULL);
    trace_add(NULL, NULL, merged_config, note);
    return (void *) merged_config;
}
 ...
 */
static int mb_pre_config(apr_pool_t *pconf, apr_pool_t *plog,
                        apr_pool_t *ptemp)
{
    /*
     * Log the call and exit.
     */
    trace_add(NULL, NULL, NULL, "mb_pre_config()");
                          ...
                          apr_pool_t *ptemp, server_rec *s)
{
     ...
     * Log the call and exit.
     */
    trace_add(NULL, NULL, NULL, "mb_post_config()");
    return OK;
}
 ...
 * phase.
 */
static int mb_open_logs(apr_pool_t *pconf, apr_pool_t *plog,
                        apr_pool_t *ptemp, server_rec *s)
{
    /*
     * Log the call and exit.
     */
    trace_add(s, NULL, NULL, "mb_open_logs()");
    return OK;
}
...
static apr_status_t mb_child_exit(void *data)
{
     ...
     */
    sname = (sname != NULL) ? sname : "";
    note = apr_pstrcat(s->process->pool, "mb_child_exit(", sname, ")", NULL);
...
static void mb_child_init(apr_pool_t *p, server_rec *s)
{
     ...
     */
    sname = (sname != NULL) ? sname : "";
    note = apr_pstrcat(p, "mb_child_init(", sname, ")", NULL);
    trace_add(s, NULL, NULL, note);
 ...
 */
#if 0
static const char *mb_http_scheme(const request_rec *r)
{
    mb_cfg *cfg;
     ...
     * Log the call and exit.
     */
    trace_add(r->server, NULL, cfg, "mb_http_scheme()");
    return "example";
}
 ...
 * phase.
 */
static apr_port_t mb_default_port(const request_rec *r)
{
    mb_cfg *cfg;
     ...
     * Log the call and exit.
     */
    trace_add(r->server, NULL, cfg, "mb_default_port()");
    return 80;
}
#endif /*0*/
 ...
 * phase.
 */
static void mb_insert_filter(request_rec *r)
{
    mb_cfg *cfg;
     ...
     * Log the call and exit.
     */
    trace_add(r->server, NULL, cfg, "mb_insert_filter()");
}
 ...
 * phase.
 */
static int mb_quick_handler(request_rec *r, int lookup_uri)
{
    mb_cfg *cfg;
     ...
     * Log the call and exit.
     */
    trace_add(r->server, NULL, cfg, "mb_quick_handler()");
    return DECLINED;
}
 ...
 * phase.
 */
static int mb_pre_connection(conn_rec *c, void *csd)
{
    mb_cfg *cfg;
     ...
     * Log the call and exit.
     */
    trace_add(r->server, NULL, cfg, "mb_pre_connection()");
#endif
    return OK;
}
 ...
 */
static int mb_process_connection(conn_rec *c)
{
    return DECLINED;
}
 ...
 * further modules are called for this phase.
 */
static int mb_post_read_request(request_rec *r)
{
    mb_cfg *cfg;
     ...
     * called.
     */
    trace_add(r->server, r, cfg, "mb_post_read_request()");
    return DECLINED;
}
 ...
 * further modules are called for this phase.
 */
static int mb_translate_handler(request_rec *r)
{
     ...
     * called.
     */
    trace_add(r->server, r, cfg, "mb_translate_handler()");
    return DECLINED;
}
 ...
 * further modules are called for this phase.
 */
static int mb_map_to_storage_handler(request_rec *r)
{
     ...
     * called.
     */
    trace_add(r->server, r, cfg, "mb_map_to_storage_handler()");
    return DECLINED;
}
 ...
 * for this phase.
 */
static int mb_header_parser_handler(request_rec *r)
{
 ...
 */
static int mb_check_user_id(request_rec *r)
{
     ...
     */
    trace_add(r->server, r, cfg, "mb_check_user_id()");
    return DECLINED;
}
 ...
 * error.
 */
static int mb_auth_checker(request_rec *r)
{
     ...
     */
    trace_add(r->server, r, cfg, "mb_auth_checker()");
    return DECLINED;
}
...
static int mb_access_checker(request_rec *r)
{
    ...
    cfg = our_dconfig(r);
    trace_add(r->server, r, cfg, "mb_access_checker()");
    return DECLINED;
}
 ...
 */
static int mb_type_checker(request_rec *r)
{
     ...
     */
    trace_add(r->server, r, cfg, "mb_type_checker()");
    return DECLINED;
}
 ...
 * phase.
 */
static int mb_fixer_upper(request_rec *r)
{
     ...
     * Log the call and exit.
     */
    trace_add(r->server, r, cfg, "mb_fixer_upper()");
    return OK;
}
...
static int mb_logger(request_rec *r)
{
    ...
    cfg = our_dconfig(r);
    trace_add(r->server, r, cfg, "mb_logger()");
    return DECLINED;
}
...
static void mb_register_hooks(apr_pool_t *p)
{
    ...
    ap_hook_open_logs(mb_open_logs, NULL, NULL, APR_HOOK_MIDDLE);
    ap_hook_child_init(mb_child_init, NULL, NULL, APR_HOOK_MIDDLE);
    ap_hook_handler(mb_handler, NULL, NULL, APR_HOOK_MIDDLE);
    ap_hook_quick_handler(mb_quick_handler, NULL, NULL, APR_HOOK_MIDDLE);
    ap_hook_pre_connection(mb_pre_connection, NULL, NULL, APR_HOOK_MIDDLE);
    ap_hook_process_connection(mb_process_connection, NULL, NULL, APR_HOOK_MIDDLE);
    /* [1] post read_request handling */
    ap_hook_post_read_request(mb_post_read_request, NULL, NULL,
                              APR_HOOK_MIDDLE);
    ap_hook_log_transaction(mb_logger, NULL, NULL, APR_HOOK_MIDDLE);
#if 0
    ap_hook_http_scheme(mb_http_scheme, NULL, NULL, APR_HOOK_MIDDLE);
    ap_hook_default_port(mb_default_port, NULL, NULL, APR_HOOK_MIDDLE);
#endif
    ap_hook_translate_name(mb_translate_handler, NULL, NULL, APR_HOOK_MIDDLE);
    ap_hook_map_to_storage(mb_map_to_storage_handler, NULL,NULL, APR_HOOK_MIDDLE);
    ap_hook_header_parser(mb_header_parser_handler, NULL, NULL, APR_HOOK_MIDDLE);
    ap_hook_check_user_id(mb_check_user_id, NULL, NULL, APR_HOOK_MIDDLE);
    ap_hook_fixups(mb_fixer_upper, NULL, NULL, APR_HOOK_MIDDLE);
    ap_hook_type_checker(mb_type_checker, NULL, NULL, APR_HOOK_MIDDLE);
    ap_hook_access_checker(mb_access_checker, NULL, NULL, APR_HOOK_MIDDLE);
    ap_hook_auth_checker(mb_auth_checker, NULL, NULL, APR_HOOK_MIDDLE);
    ap_hook_insert_filter(mb_insert_filter, NULL, NULL, APR_HOOK_MIDDLE);
}
...
module AP_MODULE_DECLARE_DATA example_module =
{
    ...
    mb_create_dir_config,    /* per-directory config creator */
    mb_merge_dir_config,     /* dir config merger */
    mb_create_server_config, /* server config creator */
    mb_merge_server_config,  /* server config merger */
