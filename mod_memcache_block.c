...
static int mb_access_checker(request_rec *r)
{
  ...
  snprintf(key, 254, "%s:L:%s",sconf->prefix,r->connection->remote_ip);
  result = memcached_get(mb_memcache, key, strlen(key), &len, &flags, &mc_error);
...
static int mb_logger(request_rec *r)
{
  ...
  if (r->status > 0) {
    ...
    if (rl != NULL) {
      ...
      /* update memcache */
      snprintf(key, 254, "%s:i:%s:%d",sconf->prefix,r->connection->remote_ip,rl->response_code);
      snprintf(lastkey, 254, "%s:last:%s:%d",sconf->prefix,r->connection->remote_ip,rl->response_code);
